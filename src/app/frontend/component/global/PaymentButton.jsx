...
const PaymentButton = ({ text = "", onClick }) => {
    ...
    const bgcolor =
        __text.includes("stripe")
            ? "bg-violet-600"
            : __text.includes("paypal")
                ? "bg-blue-600"
                : __text.includes("wallet")
                    ? "bg-green-600"
                    : "bg-blue-500";

    return (
        <>
            {__text.includes("select") || __text.includes("creating") ? (
                <BookingButton onClick={handelAction} text={text} />
            ) : (
                <button
                    onClick={handelAction}
                    className="button"
                >
                    <span className={`button-decor ${bgcolor}`}></span>

                    <div className="button-content">
                        <div className={`button__icon ${bgcolor}`}>
                            {__text.includes("stripe") ||
                                __text.includes("paypal") ||
                                __text.includes("wallet") ? (
                                <img
                                    src={`../${__text.includes("stripe")
                                        ? "stripe.webp"
                                        : __text.includes("paypal")
                                            ? "paypal.png"
                                            : "wallet.webp"
