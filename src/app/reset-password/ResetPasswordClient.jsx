...
import { useRestPassword } from "../frontend/store/resetPasswordStore";
...
export default function ResetPasswordClient() {
  ...
  const { resetPassword, message, loading, isNext } = useRestPassword();
  ...
  async function handleSubmit(e) {
    ...
    if (!token) {
      alert("Invalid password setup link.");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-20">
      ...
      >
        ...
        {!isNext && message && (
          <h5 className="text-center mt-5">{message}</h5>
        )}
      </form>
    </main>
  );
