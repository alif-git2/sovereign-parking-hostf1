...
export async function GET(request) {
  try {
    ...
    const pages = await SeoPageSetting.find({}).sort({ path: 1 }).lean();
    ...
    return handleError(error, "Failed to load SEO pages.");
  }
}
...
export async function POST(request) {
  try {
    ...
    const page = await SeoPageSetting.create(payload);
    ...
    return handleError(error, "Failed to create page SEO.");
  }
}
