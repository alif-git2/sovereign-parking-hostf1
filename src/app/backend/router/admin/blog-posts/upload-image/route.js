...
function requireAdmin(request) {
  ...
  if (role !== "admin") {
    const error = new Error("Only admin can upload blog images.");
...
async function uploadToCloudinary(file) {
  ...
  const publicId = `blog-${Date.now()}-${randomUUID()}`;
...
function handleError(error, fallbackMessage = "Blog image upload failed.") {
